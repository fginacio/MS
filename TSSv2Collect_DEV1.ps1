...
Switch ($MENU)
	...
	Write-Host "=> Illegal characters/symbols: #<>*_/\{}$+%`|=@\" -ForegroundColor Yellow -BackgroundColor DarkGray
    $CaseNumber = Read-Host -Prompt "Please enter relevant case number or Service tag"
    $destinationZip = "$dell$CaseNumber_BSOD.zip"
    $destinationZip2 = "$dell$CaseNumber_Minidump.zip"
        if ([string]::IsNullOrWhiteSpace($CaseNumber))
            {
    ...
    #Compressing logs
        $sourceFolder1 = "C:\Windows\Minidump\" 
        $sourceFolder2 = "C:\windows\memory.dmp"
        [Reflection.Assembly]::LoadWithPartialName( "System.IO.Compression.FileSystem" )
        [System.IO.Compression.ZipFile]::CreateFromDirectory($sourceFolder1, $destinationZip)
        [System.IO.Compression.ZipFile]::CreateFromDirectory($sourceFolder2, $destinationZip2)
        Write-Host "Compressing $sourceFolder folder to $destinationZip. This might take a while."
        Start-Sleep -s 5 #give some time for logging to complete before starting zip

    #Remove-Item "C:\Dell\SDP_*" -recurse -force -ErrorAction Ignore
    $Shell = New-Object -ComObject "WScript.Shell"
    $Button = $Shell.Popup("Logs available at $dell$CaseNumber_BSOD.zip", 0, "Collection Successfull", 0)
    Start-Sleep -Seconds 2
    DisplayMenu
    }
        m {
